...
#     pixel_values = torch.stack([example["target"] for example in examples])
#     pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()

#     conditioning_pixel_values = torch.stack([example["hint"] for example in examples])
#     conditioning_pixel_values = conditioning_pixel_values.to(
#         memory_format=torch.contiguous_format
#     ).float()

#     input_ids = torch.stack([ example["caption"] for example in examples])
...
def start_train(args, train_dataset: torch.utils.data.Dataset):
    ...
    # load models to use
    tokenizer, noise_scheduler, text_encoder, vae, unet = load_models(
    ...
    if args.optimizer == "AdamW8bit":
        ...
        try:
            ...
            raise ImportError(
                "To use Lion, please install the lion_pytorch library: `pip install lion_pytorch`."
            )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                encoder_hidden_states = text_encoder(
                    tokenize_captions(tokenizer, batch["captions"])
                )[0]
